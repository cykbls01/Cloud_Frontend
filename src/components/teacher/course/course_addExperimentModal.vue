<template>
  <Modal
    ...
    <div>
      ...
      <Row justify="end" class="code-row-bg" style="padding-top:2%">
        <Col span="7" style="text-align: center;margin-top:1%">
          <span class="input-item">实验延期时间</span>
        </Col>
        <Col span="17">
      ...
      <Row justify="end" class="code-row-bg" style="padding-top:2%">
        <Col span="7" style="text-align: center;margin-top:1%">
          <span  class="input-item">实验说明</span>
        </Col>
        <Col span="17">
      ...
      <Row justify="end" class="code-row-bg" style="padding-top:2%">
        <Col span="7" style="text-align: center;margin-top:1%">
          <span  class="input-item">实验资源</span>
        </Col>
        <Col span="17">
          <Upload
      ...
      <Row justify="end" class="code-row-bg" style="padding-top:2%">
        <Col span="7" style="text-align: center;margin-top:1%">
          <span class="input-item">开启互评</span>
        </Col>
        <Col span="17">
          <Switch size="large" style="margin-top:1.5%" v-model="enablePeerReview">
            <span class="on-off" slot="open">ON</span>
            <span class="on-off" slot="close">OFF</span>
            <!--TODO:// 开关互评的样式-->
          </Switch>
        </Col>
      </Row>
      <Row justify="end" class="code-row-bg" style="padding-top:2%" v-show="enablePeerReview">
        <Col span="7" style="text-align: center;margin-top:1%">
          <span  class="input-item">互评细则</span>
        </Col>
        <Col span="17">
          <Input v-model="peerReviewDescription" style="width:300px" type="textarea" :rows="4" placeholder="请输入互评细则..." />
        </Col>
      </Row>
      <Row justify="end" class="code-row-bg" style="padding-top:2%" v-show="enablePeerReview">
        <Col span="7" style="text-align: center;margin-top:1%">
          <span  class="input-item">互评截止时间</span>
        </Col>
        <Col span="17">
          <DatePicker v-model="peerReviewDeadline" :options="peerReviewDeadlineOption" type="datetime" placeholder="请选择日期时间" style="width: 300px"></DatePicker>
        </Col>
      </Row>
      <Row justify="end" class="code-row-bg" style="padding-top:2%" v-show="enablePeerReview">
        <Col span="7" style="text-align: center;margin-top:1%">
          <span  class="input-item">申诉截止时间</span>
        </Col>
        <Col span="17">
          <DatePicker v-model="appealDeadline" :options="appealDeadlineOption" type="datetime" placeholder="请选择日期时间" style="width: 300px"></DatePicker>
        </Col>
      </Row>
    </div>
    <div slot="footer">
  ...
  methods: {
    ...
    judgeParams (params) {
      ...
      if (params.course_id === '' || typeof (params.course_id) === 'undefined') {
        ...
        this.showNoticeWarning('实验延期时间错误', '实验延期时间不能为空')
        flag = false
      ...
      let listTime = [params.start_time, params.end_time, params.deadline, params.peer_assessment_deadline, params.appeal_deadline]
      let listTimeName = ['开始时间', '结束时间', '实验延期时间', '互评截止时间', '申诉截止时间']
      ...
      this.$http.post(this.postUrl, formData, config)
        .then(res => {
          if (res.data.code === 1001) {
            this.showNoticeSuccess('操作成功', '已添加实验' + this.name)
            /*
