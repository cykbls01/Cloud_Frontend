...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            switch (params.row.operate_type) {
              ...
              case 4: type = '创建容器'; break
              case 5: type = '修改容器'; break
              case 6: type = '申请共享容器'; break
                    ...
                    if (params.row.status === 0) {
                      switch (params.row.operate_type) {
                        ...
                        case 3:
                          ...
                          break
                        case 4:
                          this.modalTitle = '创建容器'
                          this.modalType = 'con'
                          this.modallist.push({name: '容器名称', value: vm.podname})
                          this.modallist.push({name: '镜像名称', value: vm.image})
                          this.modallist.push({name: '端口号', value: vm.port})
                          break
                        case 5:
                          this.modalTitle = '修改容器'
                          this.modalType = 'con'
                          this.modallist.push({name: '容器名称', value: vm.podname})
                          this.modallist.push({name: '镜像名称', value: vm.image})
                          this.modallist.push({name: '端口号', value: vm.port})
                          break
                        case 6:
                          this.modalTitle = '申请共享容器'
                          this.modalType = 'share'
                          this.modallist.push({name: '容器名称', value: vm.pod_name})
                          this.modallist.push({name: '镜像名称', value: vm.con_name})
                          this.modallist.push({name: '数据库名', value: vm.db_name})
                          break
                      }
                      this.opModal = true
                    } else {
  ...
  methods: {
    ...
    submit () {
      let data = {}
      if (this.modalType === 'con') {
        let params = {
          applyId: this.applicationId
        }
        axios.post(go.gourl + '/createContainer', params,
          {
            headers: {
        ...
        axios.post(go.gourl + '/createSharePod', params,
          {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded',
              'Authorization': this.$cookie.get('token')
